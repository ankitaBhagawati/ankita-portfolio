  ...
  return (
    <>
      <motion.nav
        ...
        className="fixed inset-x-0 top-0 z-50 h-16 flex items-center justify-between px-4 md:px-12 backdrop-blur-xl"
      ...
      >
        ...
        >
          <div className="relative flex-shrink-0">
            ...
            <div className="relative w-9 h-9 rounded-full p-[2px] bg-gradient-to-br from-[#5b8fff] to-[#a78bfa]">
              <div className="w-full h-full rounded-full overflow-hidden flex items-center justify-center"
                style={{ background: "var(--bg)" }}>
                <img src="/bagwitty.jpeg" alt="Ankita Bhagawati"
          ...
          <div className="flex flex-col leading-none text-left">
            ...
            <span className="text-[0.62rem] font-medium tracking-[0.04em] mt-[3px]"
        ...
        <div className="hidden md:flex items-center gap-1">
          {links.map((l) => (
            <a key={l.href} href={l.href}
              className={cn(
                "nav-link text-sm font-medium px-3 py-1.5 rounded-full transition-all duration-200",
                active === l.href.replace("#", "") ? "active" : ""
              )}>
              {l.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="md:hidden p-1.5" style={{ color: "var(--text-secondary)" }}
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            style={{ background: "var(--bg)" }}>
            <button className="absolute top-5 right-6 transition-colors"
              style={{ color: "var(--text-muted)" }}
            ...
            {links.map((l, i) => (
              <motion.a key={l.href} href={l.href}
                ...
                onClick={() => setMenuOpen(false)}
                className="font-display text-3xl font-bold section-title hover:gradient-text transition-all">
                {l.label}
              </motion.a>
            ))}
          </motion.div>
