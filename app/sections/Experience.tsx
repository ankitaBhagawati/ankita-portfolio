  ...
  return (
    <section id="experience" className="relative z-10 py-24">
      <div className="max-w-6xl mx-auto px-4 md:px-12">
        ...
        <Reveal><p className="text-[0.68rem] tracking-[0.22em] text-[#5b8fff] uppercase font-bold mb-2">Career Timeline</p></Reveal>
        <Reveal delay={0.05}>
        ...
        {/* Impact strip */}
        <Reveal delay={0.1}>
          <div className="flex flex-wrap gap-2 items-center mb-14 p-4 rounded-xl
            border border-[rgba(91,143,255,0.18)] bg-[rgba(91,143,255,0.06)] mb-14">
            ...
            {resume.topImpact.map((item) => (
              <span key={item}
                ...
                <Zap size={10} className="text-[#5b8fff]" />
                {item}
              </span>
            ))}
          </div>
            ...
            <Reveal key={exp.id} delay={i * 0.08}>
              <div
                className="rounded-2xl border border-white/[0.08] bg-white/[0.04] overflow-hidden
                  hover:border-[rgba(91,143,255,0.3)] hover:shadow-[0_6px_36px_rgba(91,143,255,0.07)]
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-[0.97rem] text-white mb-0.5">{exp.role}</div>
                    <div className="text-sm text-[#5b8fff] font-medium mb-1.5">
                      {exp.company}
                      {exp.location && (
                        <span className="text-white/40 font-normal ml-2 inline-flex items-center gap-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {exp.current && (
                        <span className="px-2 py-0.5 rounded text-[0.68rem] font-medium
                          bg-[rgba(52,211,153,0.1)] border border-[rgba(52,211,153,0.3)] text-[#34d399]">
                          ● Current
                        </span>
                      )}
                      <span className="px-2 py-0.5 rounded text-[0.68rem] bg-white/5 border border-white/10 text-white/50">
                        {exp.period}
                      </span>
                      <span className="px-2 py-0.5 rounded text-[0.68rem] bg-white/5 border border-white/10 text-white/50">
                        {exp.duration}
                  ...
                  <ChevronDown
                    size={18}
                    className={`text-white/40 flex-shrink-0 transition-transform duration-300 ${open === exp.id ? "rotate-180" : ""}`}
                  />
                </button>
                      ...
                      <div className="px-6 pb-6 border-t border-white/[0.06]">
                            ...
                            {exp.metrics.map((m) => (
                              <span key={m}
                                className="px-2.5 py-1 rounded-md text-xs font-bold
                                  bg-[rgba(91,143,255,0.12)] border border-[rgba(91,143,255,0.25)] text-[#5b8fff]">
                                {m}
                              </span>
                            ))}
                          </div>
                        ...
                        <ul className="flex flex-col gap-3 pt-3">
                          {exp.bullets.map((b, bi) => (
                            <li key={bi} className="flex gap-3 text-[0.86rem] text-white/60 leading-relaxed">
                              <span className="text-[#5b8fff] flex-shrink-0 mt-0.5 text-xs">→</span>
                              {b}
                            </li>
                          ))}
                        </ul>
